...
#:endif
  ...
  subroutine writeAdditionalAutotestTag(fileName, tPeriodic, totalEnergy, repEnergy, repDerivs,&
      & tStress, repStress, taggedWriter)
    ...
    call taggedWriter%write(fd, tagLabels%egyTotal, totalEnergy)
    call taggedWriter%write(fd, tagLabels%egyRep, repEnergy)

    if (allocated(repDerivs)) then
      call taggedWriter%write(fd, tagLabels%forceRep, -repDerivs)
    end if

    if (tPeriodic) then
      if (tStress) then
        call taggedWriter%write(fd, tagLabels%stressRep, repStress)
      end if
    end if
    close(fd)
