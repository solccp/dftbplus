...
contains
  ...
  subroutine runDftbPlus(env)
    ...
    real(dp) :: massx
    type(TRanlux), allocatable :: randomFreq
    type(TRandomGenPool) :: randGenPool
    type(TTaggedWriter) :: taggedWriter2
  ...
  #:endif
    ...
    if (tWriteAutotest) then
      ...
      call writeAdditionalAutotestTag(autotestTag, tPeriodic,&
          & energy%Etotal, energy%Erep, repDerivs, tStress, repStress, taggedwriter)
    end if
    ...
    if (env%tGlobalMaster .and. tDerivs) then
      call TTaggedWriter_init(taggedWriter2)
...
!######################################################
      ...
      call dgemm('N', 'N', n3, n3-nindep, n3-nindep, 1.0_dp, basis(1,nindep+1), n3, newhess(nindep+1,nindep+1), &
          & n3, 0.0_dp, pDynMatrix(1,nindep+1), n3)
  ...
  !    write(*,'(6f8.2)') eigenValues(1:nindep)*Hartree__cm
      ...
      if (tNormalModes) then
          ...
          write(fdNormalModes,*)"---------------------------------------------------------------------"
          do ii=1, nAtom
              write(fdNormalModes,'(i5,6x,i5,13x,i1,4x,3f12.6)') ii, getAtomicNumberFromSymbol(speciesName(species(ii))), &
              & 0,(0.529177208300000035*coordcm(jj,ii),jj=1,3)
          end do
          write(fdNormalModes,*)"---------------------------------------------------------------------"
          ...
          do ii=nindep+1, n3, 3
              ...
              do jj=1, nAtom
                  write(fdNormalModes,'(2i4,3(2x,3f7.2))')jj,getAtomicNumberFromSymbol(speciesName(species(jj))), &
                  &((pDynMatrix(3*(jj-1)+licz,iCount),licz=1,3),iCount=ii,kk)
              end do
          end do
          write(fdNormalModes,*) '' 
          close(fdNormalModes)
      ...
      if (tWriteAutotest) then
          ...
          call taggedWriter2%write(info, tagLabels%frequencies, eigenValues(nindep+1:))
          close(info)
      end if
...
#:endif
  ...
  subroutine getGradients(env, sccCalc, tExtField, isXlbomd, nonSccDeriv, EField, rhoPrim,&
    ...
    call getERepDeriv(repDerivs, coord, nNeighbourRep, neighbourList%iNeighbour, species, pRepCont,&
        & img2CentCell)
  ...
  subroutine getStress(env, sccCalc, thirdOrd, tExtField, nonSccDeriv, rhoPrim, ERhoPrim, qOutput,&
      ...
      & totalStress, repStress, totalLatDeriv, intPressure, iRhoPrim, solvation, dispersion, halogenXCorrection)
    ...
    call getRepulsiveStress(repStress, coord, nNeighbourRep, neighbourList%iNeighbour, species,&
